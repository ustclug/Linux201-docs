...
SSH 连接由本地的 ssh 客户端发起，连接到远端服务器的 22 端口，建立连接后，你在终端输入的内容默认都会发送到远端服务器。

但有时候，我们需要让本地 ssh 客户端立即响应一些特殊操作，比如断开连接、挂起会话、配置端口转发等，这时就可以用“转义序列”来实现。

转义序列的用法是：在新的一行先按回车，然后输入 `~`，后面的内容就会被本地 ssh 客户端识别为命令，而不是发给远端服务器。
...
!!! tip "使用注意"
    ...
    - 在多层 SSH 跳板（多次 ssh 嵌套）时，每多一层，需要多输入一个 ~，如 ~~. 断开第二层连接。
