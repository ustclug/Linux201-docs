...
!!! tip "使用 `aio` 避免工作进程阻塞"
    ...
    [`aio`](https://nginx.org/en/docs/http/ngx_http_core_module.html#aio) 指令支持基于 Linux 内核 AIO 和线程池这两种实现。添加 `aio threads;` 即可。默认**每个工作进程的**线程池为 32 线程，最多排队 65536 个请求，可以使用 [`thread_pool`](https://nginx.org/en/docs/ngx_core_module.html#thread_pool) 指令调整。以下是科大镜像站在开启 `aio threads` 前后的请求主页的 HTTP 延迟情况，可以看到效果十分显著：
...
!!! tip "使用 mkcert 创建 CA 证书"
    ...
    ??? note "我还是想用 OpenSSL"
        ...
        openssl req -x509 -new -noenc -key ca.key -sha256 -days 3650 -subj "/CN=ExampleCA" -out ca.crt
        # 创建需要签名的证书密钥与证书签名请求（CSR）
        # 此处额外添加了需要向证书添加的扩展信息（以 SAN——Subject Alt Name 为例）
        openssl genrsa -out cert.key 4096
        openssl req -new -key cert.key -subj "/CN=example.com" -out cert.csr \
          -addext "subjectAltName=DNS:example.com,DNS:*.example.com,IP:127.0.0.1"
        # 进行签名，生成证书
        openssl x509 -req -in cert.csr -CA ca.crt -CAkey ca.key -CAcreateserial -out cert.crt -days 825 -sha256 -copy_extensions copy
        # 验证
...
可以看到，`rewrite` 指令的第一个参数是一个正则表达式，用于匹配请求的 URI。第二个参数是改写后的 URI，可以使用正则表达式中的捕获组（例如 `$1`、`$2` 等，也支持 `(?<name>pattern)` 格式的 PCRE 命名捕获组）来引用匹配到的内容。第三个参数是可选的 flag：
