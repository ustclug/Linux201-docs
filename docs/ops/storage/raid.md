...
!!! comment "@taoky: 硬件 RAID，md/LVM，还是 ZFS？"
    ...
    - 软件 RAID 方案的监控与运维操作更方便
        ...
        - 而查看软件 RAID 的状态就方便得多
    - 不受制于闭源、无法调试的固件
        - 我们曾经遇到过在某台旧服务器上，硬件 RAID 卡固件多次崩溃的问题，此时整个阵列（包括系统盘）都会掉线，并且没有人知道为什么——开放的软件 RAID 方案至少还有调试的机会
        - 商业 SAN 方案也有类似的问题——我们也遇到过 SAN 的管理口启动一会之后就直接坏掉的情况，而且无法处理（同样，没有人知道为什么）
        - 即使硬件 RAID 卡支持 HBA/Non-RAID/JBOD 模式，可以将硬盘直接暴露给操作系统，但是其硬件与固件的复杂度仍然高于 HBA 卡，更容易损坏，是很容易被忽视的单点故障源（科大镜像站在 2026 年 7 月就因处在 JBOD 模式的硬件 RAID 卡故障而服务中断）
    - 文件系统比块设备（和阵列卡）更懂数据
