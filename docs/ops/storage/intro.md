...
一般来讲，单块机械硬盘的性能取决于 RPM（转速）。在估算时可以认为顺序读写带宽在 100 MB/s 左右，4K 随机读写带宽在 1 MB/s 左右，IOPS[^1] 在 100 左右。而单块固态硬盘的顺序读写带宽在 500 MB/s (SATA) 或 1500 MB/s (NVMe) 以上，4K 随机读写带宽在 50 MB/s 左右，IOPS 在 10000 左右。固态硬盘相比机械硬盘，因为不需要机械寻道，同时处理多个请求的性能好得多。对于企业级硬盘，这些性能指标可能会更高。

在性能测试时，可能会看到队列深度（Queue Depth，QD）这个词，可以认为是同时的并发度。SATA 协议只支持一个队列，深度为 32，而 NVMe 支持 65535 个队列，每个队列最大深度 65535，但是一般来说达不到这个水平。厂商可能会同时提供 QD1 和 QD32 等下的参考性能指标。

此外，对固态硬盘，在购买时还需要注意每个存储单元中存储的数据位数。SLC、MLC、TLC、QLC 分别对应在一个单元（Cell）中存储 1、2、3、4 位数据，存储得越多，成本越低、容量越大，但是对应写入性能越差、寿命越短。在家用场景下，TLC 或者 QLC SSD 比较常见。另外不少 TLC 或者 QLC 的固态硬盘固件会使用一部分空间用作 SLC 缓存，因此在进行性能测试的时候很容易会发现最开始的时候写入性能很好，但是在缓存耗尽后，性能就会明显降低。对 QLC，写入性能可能会低于 100 MB/s。
...
!!! note "关注存储的可用空间比例"
    ...
    - 固态硬盘：可用空间不足会导致没有足够的空区块改写内容，因此可能不得不大量重复擦写已有的区块，加速磨损。此外对使用了 SLC 缓存的固态硬盘，可用空间不足时 SLC 缓存大小也会小很多，导致性能降级。
...
服务器一般都配备了硬件 RAID 卡，如果需要使用软件 RAID，需要设置 RAID 卡或对应磁盘到 HBA 模式（也叫 JBOD/Non-RAID/直通 模式）。此外，如果不打算使用硬件 RAID 的功能，也可以替换为功能更简单（不包含诸如电池、缓存、专用处理器等）的 HBA 卡，减小因硬件 RAID 卡故障导致机器宕机的概率。
...
在实际运行的系统中，可以使用 `iostat -x 1` 命令查看每块磁盘设备每秒的性能信息。`iostat` 会输出磁盘读写、Trim（discard）和 Flush 操作的性能。以读取为例，包括每秒的操作次数（`r/s`）、数据量（`rkB/s`）、合并掉的请求数（`rrqm/s`）、合并掉的请求占的比例（`%rrqm`）、处理请求的时间（`r_await`，毫秒）与请求的平均大小（`rareq-sz`，KiB）。此外还有磁盘队列的平均深度（`aqu-sz`）和磁盘利用率（`%util`）。

`iostat`，配合[下文使用 `fio` 的测试](#benchmark)，可以辅助判断：

- 磁盘 I/O 是不是当前的瓶颈（如果磁盘的利用率低、读写操作次数和数据量都不高，那么程序可能忙在 CPU 或者网络上）
- 磁盘是否忙在 Trim 或者 Flush 操作上（如果当前没有重 I/O 操作，但是系统卡顿；或者程序频繁进行 sync 操作导致 I/O 慢）
- 当前负载的瓶颈在 IOPS 还是带宽上（观察操作次数与数据量）
- 并发进行磁盘 I/O 是否有机会提升 I/O 性能（队列深度，特别是对 NVMe 设备）
...
[`/etc/fstab`][fstab.5] 是 Linux 系统中用于配置文件系统挂载的文件。
...
在现代 Linux 系统中，systemd 的 [systemd-fstab-generator][systemd-fstab-generator.8] 会从 `/etc/fstab` [生成](../service.md#generator)出与挂载点相关的 unit。generator 和挂载点 unit（[systemd.mount][systemd.mount.5]、[systemd.swap][systemd.swap.5]）也接受一些特殊的选项，用来控制生成出的 unit 的行为。

大多需要让 systemd 特殊配置的挂载点都与网络有关。对网络挂载点，一个常见的重要选项是 `_netdev`，它表示这个挂载点需要网络连接，systemd 会配置启动时在网络配置好之后才挂载。这个选项在挂载基于网络的存储时非常有用。此外，如果不希望在开机时就挂载，而是在需要时才挂载上，或者想在不需要时自动卸载，可以使用内核的 [autofs](https://docs.kernel.org/filesystems/autofs.html) 特性，systemd 将其包装为了 [systemd.automount][systemd.automount.5]，在 fstab 配置中添加 `x-systemd.automount` 与 `x-systemd.idle-timeout=`（如果需要在不使用时自动卸载）即可。

"dump" 可以忽略（0 即可），而 "pass" 标记了文件系统检查（fsck）的顺序：0 不检查，根分区应该为 1，其他分区为 2。

在修改配置后，如果系统使用了 systemd，应当使用 `systemctl daemon-reload` 来让 systemd 重新加载所有的挂载点 unit。
否则 `mount -a` 之后，由于 systemd 维护的 unit 没有变化，它可能会帮你非预期地重新挂载。
