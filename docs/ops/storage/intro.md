...
在实际运行的系统中，可以使用 `iostat -x 1` 命令查看每块磁盘设备每秒的性能信息。`iostat` 会输出磁盘读写、Trim（discard）和 Flush 操作的性能。以读取为例，包括每秒的操作次数（`r/s`）、数据量（`rkB/s`）、合并后的请求数（`rrqm/s`）、合并的请求占的比例（`%rrqm`）、处理请求的时间（`r_await`，毫秒）与请求的平均大小（`rareq-sz`，KiB）。此外还有磁盘队列的平均深度（`aqu-sz`）和磁盘利用率（`%util`）。
