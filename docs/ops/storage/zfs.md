...
- 克隆一个快照并不会改变该快照的从属关系，即克隆出新数据集后，源快照仍然隶属于原始数据集。
...
```shell title="将一个快照通过 SSH 发送至另一台 ZFS 存储服务器上"
...
??? example "案例：通过调节参数降低镜像站的磁盘负载[^ustc-zfs]"
    ...
    我们将完整的镜像站调优故事[发布在了 LUG @ USTC 社团主页][ustc-zfs]上，感兴趣的读者可以前往阅读。
...
<!-- markdownlint-disable MD053 -->
  ...
  [^ustc-zfs]: [镜像站 ZFS 实践][ustc-zfs] - LUG @ USTC
  [ustc-zfs]: <https://lug.ustc.edu.cn/planet/2024/12/ustc-mirrors-zfs-rebuild/>
