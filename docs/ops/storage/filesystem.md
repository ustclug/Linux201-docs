...
Subvolume 是 Btrfs 的一个重要概念，可以看作是 Btrfs 的「子文件系统」，类似于 [ZFS](./zfs.md) 的 dataset。与目录不同，Subvolume 是独立的，可以有自己的挂载点。同时 subvolume 共享同一个 Btrfs 文件系统的空间，不需要手动分配空间。
...
尽管 top-level subvolume 也可以正常存储数据，但是即使不将创建的 Btrfs 分区安装 Linux，我们仍然**建议始终创建一个新的 subvolume（例如 `@`）用于数据存储**，以方便诸如快照、数据传输等操作。
...
创建出来的 subvolume 在父 subvolume 上以目录的形式存在，不过这些 subvolume 的设备号是不同的，因此通过判断设备号，应用可以区分当前的文件操作是否还在相同的 subvolume 上（例如 `rsync` 的 `-x` 参数），避免跨过 subvolume 边界误操作：

```console
$ stat -c '%d %n' /media/btrfs/ /media/btrfs/subvol1/ /media/btrfs/subvol2/ /media/btrfs/subvol3/
221 /media/btrfs/
222 /media/btrfs/subvol1/
224 /media/btrfs/subvol2/
...
$ sudo mount -o subvolid=257 btrfs.img /media/btrfs2  # 挂载 subvol2（ID 257）
$ mount | grep btrfs.img
/path/to/btrfs.img on /media/btrfs type btrfs (rw,relatime,ssd,discard=async,space_cache=v2,subvolid=5,subvol=/)
...
可以修改在不添加参数时默认挂载的 subvolume，以下在 subvol1 下又创建了 nestedvol1，然后将其作为默认的 subvolume：
...
还记得 top-level subvolume 的 ID 永远是 5 吗？如果想看某个 btrfs 文件系统完整的 subvolume 的情况，直接挂载 top-level subvolume 即可，不用卸载当前的挂载：

```console
$ sudo mount -o subvolid=5 btrfs.img /media/btrfs1
$ sudo btrfs subvolume show /media/btrfs1
/
	Name: 			<FS_TREE>
（以下输出省略）
$ sudo umount /media/btrfs
...
在 subvolume 的基础上，Btrfs 支持了快照功能。这里的快照可能与我们熟悉的「快照」（例如虚拟机软件的快照功能）有所不同，它本质上就是和其他 subvolume 以 CoW 形式共享数据的 subvolume。让我们用上文创建的 `btrfs.img` 试一试吧：

```console
$ sudo mount -o subvolid=5 btrfs.img /media/btrfs
$ echo "test1" > /media/btrfs/subvol1/test
$ sudo btrfs subvolume snapshot /media/btrfs/subvol1 /media/btrfs/snap1
Create a snapshot of '/media/btrfs/subvol1/' in '/media/btrfs/snap1'
```

此时 snap1 和 subvol1 都是 subvolume，它们共享数据——snap1 打下快照时，存储的是目前 subvol1 的内容。在快照创建后，snap1 和 subvol1 内部的修改不会影响到对方的数据内容：
...
这里我们可以修改「快照」的内容，在 CoW 文件系统中，修改共享的内容会被复制，而未修改的内容会被共享。**不过很多时候我们不希望快照可写，在创建快照时可以加上 `-r` 参数**。可以使用 `btrfs property get` 命令验证某个 subvolume 是否是只读的。

和 `cp` 命令类似地，当快照的目标路径是一个目录时，Btrfs 工具将在该目录下一个创建同名的 subvolume，而不是将目录本身作为 subvolume 创建出来。以下展示了创建快照、从快照还原并设置为可读写的简要步骤：

```console
$ sudo mount -o subvolid=5 btrfs.img /media/btrfs
$ mkdir /media/btrfs/snapshots
...
ro=true
$ sudo mkdir /media/btrfs/recovered
...
Btrfs 支持文件系统层面的流式发送和接收，允许以字节流的形式将一个**只读** subvolume（以下简称快照）的内容拷贝到另一个文件系统上，与 [ZFS 的 send/receive 功能](./zfs.md#send-receive)类似。比如下面这个在本地两个 btrfs 分区之间传输的例子：
...
相比于 [ZFS](./zfs.md) 的去重，Btrfs 需要依赖第三方工具扫描重复项，并将相关信息提交到内核处理。最常用的工具是 [duperemove](https://github.com/markfasheh/duperemove) 和 [bees](https://github.com/Zygo/bees)。前者适用于定时或者手动对 Btrfs（也支持 XFS）执行，而后者是专为 Btrfs 编写的 daemon，会在后台执行自动去重。可以根据实际工作负载的需求选择合适的方案。以 duperemove 为例：
