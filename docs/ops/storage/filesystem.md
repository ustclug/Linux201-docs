...
| APFS                                            | :fontawesome-solid-check:{: .yellow }（需要 [linux-apfs-rw](https://github.com/linux-apfs/linux-apfs-rw)，写入支持不稳定，不支持加密）| :fontawesome-solid-check:{: .limegreen } | :fontawesome-solid-xmark:{: .orangered } | macOS 较新版本的 CoW 文件系统。                                                                                       |
